...
internal static class LeafPetalArcRasterizer
{
    ...
    internal static HashSet<int> RasterizePolygonBalanced(
        IReadOnlyList<(double X, double Y)> polygon,
        int width,
        int height)
    {
        ...
        // Keep the existing centre-sample raster as an exact 50% tie-break. This preserves the
        // stable indexed topology while the coverage pass only fixes cells for which continuous
        // geometry provides a real majority signal.
        var centerMask =
            RasterizePolygon(
                polygon,
                width,
